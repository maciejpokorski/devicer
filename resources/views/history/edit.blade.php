                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('histories.update', $history->id) }}">
                            ...
                            <div class="mt-4">
                                ...
                                <select name="device_id" id="device_id" class="block mt-1 w-full" required>
                                    @foreach ($devices as $device)
                                        <option value={{$device->id}} {{ $device->id == $history->device->id ? 'selected' : ''}}>{{ $device->name }}</option>
                                    @endforeach 
                                </select>
                            </div>
                        @else
                            ...
                            <div class="mt-4">
                                ...
                                <select name="user_id" id="user_id" class="block mt-1 w-full" required>
                                    @foreach ($users as $user)
                                        <option value={{$user->id}} {{ $user->id == $history->user->id ? 'selected' : ''}}>{{ $user->name }}</option>
                                    @endforeach 
                                </select>
                            </div>
                        @else
                        ...
                        <div class="mt-4">
                            ...
                            <div>
                                <input type="datetime-local" id="created_at"
                                    name="created_at" value="{{ date_format($history->created_at,'Y-m-d\TH:i:s') ?? date('Y-m-d\TH:i:s') }}"> 
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <x-input id="millage_old" class="block mt-1 w-full" type="number" min="0" name="millage_old" :value="$history->millage_old" required />
                        </div>
